...
export function useTerminal(options: UseTerminalOptions = {}) {
  ...
  const initTerminal = useCallback(() => {
    ...
    const terminal = new Terminal({
      ...
      theme: {
        background: getComputedStyle(document.documentElement).getPropertyValue('--color-bg').trim() || '#0a0a0a',
        foreground: getComputedStyle(document.documentElement).getPropertyValue('--color-fg').trim() || '#e0e0e0',
        cursor: getComputedStyle(document.documentElement).getPropertyValue('--color-primary').trim() || '#00ff41',
        selectionBackground: (getComputedStyle(document.documentElement).getPropertyValue('--color-primary').trim() || '#00ff41') + '33',
        ...options.theme,
      },
